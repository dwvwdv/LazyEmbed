 ...
 *
 * Keys:
 *   f      → enter hint mode (shows badges on registered cards)
 *   <key>  → (while in hint mode) jump to that card
 *   Esc    → exit hint mode; if focus is in an input, blur it instead
 */

interface HintRegistration {
  selector: string;
  /** Override badge label. Defaults to uppercase of the key. */
  label?: string;
}

class VimiumNavigator {
  private readonly registry = new Map<string, HintRegistration>();
  ...
  /**
   * Register a hint entry.
   * @param key    Single letter (case-insensitive). 'f' and 'Escape' are reserved.
   * @param selector CSS selector for the target card / section element.
   * @param label  Optional badge label override (defaults to key.toUpperCase()).
   */
  register(key: string, selector: string, label?: string): this {
    ...
    this.registry.set(k, { selector, label });
    return this;
  }
  ...
  private bindKeyboard(): void {
    document.addEventListener(
      'keydown',
      (e: KeyboardEvent) => {
        // Inside a text field: only handle Escape (to blur)
        if (this.isTyping()) {
          if (e.key === 'Escape') {
            e.preventDefault();
        ...
        if (this.hintActive) {
          const reg = this.registry.get(e.key.toLowerCase());
          if (reg) {
            e.preventDefault();
            this.activateEntry(reg);
          }
        }
      },
      { capture: true }
  ...
  private showBadges(): void {
    for (const [key, { selector, label }] of this.registry) {
      const target = document.querySelector<HTMLElement>(selector);
      if (!target) continue;

      // Cards are already position:relative in the stylesheet, but guard anyway
  ...
  private showIndicator(): void {
    ...
    // Force reflow so the CSS transition fires even on first show
    void el.offsetWidth;
  ...
  private activateEntry(reg: HintRegistration): void {
    this.exitHintMode();

    const target = document.querySelector<HTMLElement>(reg.selector);
    if (!target) return;

    target.scrollIntoView({ behavior: 'smooth', block: 'center' });

    // If the registered element is a link, navigate to it
    ...
    // Otherwise focus the first focusable input inside
  ...
  private injectStyles(): void {
    ...
    const css = `
      ...
      .vim-shortcut-pill:hover {
        color: var(--text2, #7e8a9e);
        border-color: var(--border2, #333a47);
      }
    `;
