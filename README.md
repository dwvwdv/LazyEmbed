...
### Cloudflare Workers 自動部署 (CI/CD)

本專案已配置 GitHub Actions 自動部署到 Cloudflare Workers。當您推送代碼到 `main` 分支時，將自動觸發部署。

**設置步驟**：
1. 獲取 Cloudflare API Token 和 Account ID
2. 在 GitHub 儲存庫設置中添加 Secrets
3. 推送代碼即可自動部署

📖 **完整設置指南請參閱**：[Cloudflare 自動部署設置](.github/CLOUDFLARE_SETUP.md)

📖 **其他平台部署指南請參閱**：[部署文檔](docs/deployment.md)
